...
try {
    $response = Invoke-WebRequest -Uri "http://127.0.0.1:3001/api/start_single_scrape" -Method POST -TimeoutSec 5 -ErrorAction Stop
    Write-Host "Scrape triggered successfully!" -ForegroundColor Green
    Write-Host "Check http://127.0.0.1:3001 for progress." -ForegroundColor Yellow
} catch {
